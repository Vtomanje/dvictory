<nav class="bg-gray-800" x-data="{ open: false }">
      ...
      <div class="relative flex h-16 items-center justify-between">
        ...
        <div class="absolute inset-y-0 right-0 flex items-center pr-2 sm:static sm:inset-auto sm:ml-6 sm:pr-0">
          ...
          <div class="relative ml-3" x-data="{open: false}">
            <div>
              <button x-on:click="open = true" x-on:click.away="{ open = false }" type="button" class="flex rounded-full bg-gray-800 text-sm focus:outline-none focus:ring-2 focus:ring-white focus:ring-offset-2 focus:ring-offset-gray-800" id="user-menu-button" aria-expanded="false" aria-haspopup="true">
   ...
   {{-- menu Mobile --}}
    <div class="sm:hidden" x-show="open" x-on:click.away="open = false">
        ...
        @foreach ($categories as $category)
          <a href={{route('posts.category', $category)}}" class="bg-gray-900 hover:bg-gray-700 text-white block px-3 py-2 rounded-md text-base font-medium" aria-current="page">{{$category->name}}</a> 
        @endforeach
